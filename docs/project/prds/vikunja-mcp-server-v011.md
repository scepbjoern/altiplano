...
- [x] Tasks: Mehrere Tasks suchen, Detailansicht, Erstellen (inkl. direkter Label-Zuweisung über `label_ids`), Aktualisieren der Beschreibung/Status.
...
| US-3 | Als Nutzer möchte ich gefiltert Tasks abrufen und erstellen (inkl. direkter Zuweisung von Labels). | MVP | `list_tasks` und `create_task` (mit `label_ids`) verarbeiten die Parameter und hängen Labels direkt an. |
...
| US-11 | Als Nutzer möchte ich beim Erstellen oder Aktualisieren eines Projekts einen Project Identifier setzen können. | MVP | `create_project` und `update_project` akzeptieren `identifier`. |
...
| Taskverwaltung | Suchen, Erstellen (inkl. Labels via `label_ids`), Aktualisieren, Verschieben, Erledigen. | MVP | Must | Personal User | - |
...
Siehe US-1 bis US-11. Erfolgreiche Ausführung der neuen Label-Tools ohne Datenverlust oder ungewollte Löschungen.
...
| Annahme | Vikunja übernimmt die spezifischen Identifier-Validierungen (z.B. Grossbuchstaben, alphanumerisch). | Gering | Fehlertexte der Vikunja-API im MCP-Client transparent ausgeben, anstatt doppelt zu validieren. |
| Annahme | `create_task` implementiert die Label-Zuweisung intern durch sequentielle Aufrufe der Vikunja-API (erst Task erstellen, dann Labels anhängen), da Vikunja möglicherweise keine Labels direkt im Task-Creation-Endpoint akzeptiert. | Gering | Bei der Implementierung in `server.py` sicherstellen, dass Teilfehler ordnungsgemäss abgefangen und im Return-Objekt ausgewiesen werden. |
...
| Labels in create_task | `label_ids` als optionalen Parameter für direkte Label-Zuweisung unterstützen. | MVP | - | 2 |
