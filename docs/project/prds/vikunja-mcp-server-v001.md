...
- [ ] Projekte: Farben (`hex_color`) bei `list_projects` in der Rückgabe ergänzen.
- [ ] Projekte: Neues Tool zum Aktualisieren (`update_project`) inkl. Name, Beschreibung, Farbe und Verschachtelung (`parent_project_id`).
- [ ] Task Sicherheit: Dediziertes Tool `complete_task` (obwohl `done` im Update-Tool existiert) sowie ein sicheres Tool `move_task_to_project`.
...
- [ ] Bulk-Limits: Harte technische Grenzen (max 5-10 Tasks) falls riskante Massenänderungen implementiert werden.
...
| Taskverwaltung | Suchen (Filter), Erstellen, Aktualisieren, Verschieben, Erledigen. | MVP | Must | Personal User | Inkl. Prioritäten, Daten und Reminder. |
...
|---|---|---|---|---|
| Project | Ordnerstruktur für Tasks | `id`, `title`, `description`, `hex_color`, `parent_project_id` | Enthält Tasks. Kann verschachtelt sein. | MVP |
| Bucket | Kanban-Spalten | `id`, `project_id`, `title` | Ordnet Tasks visuell in Projekten an. | Medium |
| Task | Die eigentliche Aufgabe | `id`, `project_id`, `bucket_id`, `title`, `description`, `done`, `priority`, `start_date`, `due_date` | Gehört zu Project/Bucket, hat Labels/Comments. | MVP |
...
- **Autorisierung:** Der Token sollte in Vikunja minimal provisioniert sein. Der MCP-Server bietet im MVP **keine** Delete-Endpunkte an. 
- **Destruktive Aktionen (Extended):** Sollte in Zukunft ein Lösch-Tool (Extended) eingebaut werden, so wird dieses strikt an einen manuellen Bestätigungs-Ablauf im Client gekoppelt.
...
|---|---|---|---|---|
| `update_project` Tool | Neues Tool zum Ändern von Projektname, Farbe und Verschachtelung. | MVP | Vikunja API. | 1 |
| Projekt-Ausbau | `list_projects` um Farbrückgabe ergänzen. | MVP | Vikunja API. | 2 |
| Task Sicherheits-Tools | Dediziertes `complete_task` und sicheres `move_task_to_project`. | MVP | Task-Methoden. | 3 |
| Kanban Buckets | Tools für Buckets (`create_bucket`, `update_bucket`) und Task-Zuweisung. | Medium | Vikunja API. | 4 |
| Dateianhänge | Tool zum Anhängen von Dateien an Tasks. | Medium | Vikunja API (Multipart-Form). | 5 |
| Remote HTTP MCP | Docker-Setup und HTTP-Transport (SSE) einrichten. | Medium | Reverse Proxy, Auth. | 6 |
| Task Löschen | Tool zum Löschen mit striktem zweistufigen Bestätigungs-Pattern. | Extended | Bestätigungs-Pattern. | 7 |
| Task-Beziehungen | Tool zum Setzen von Relationen (Parent/Child, Blockaden). | Extended | Vikunja API. | 8 |
