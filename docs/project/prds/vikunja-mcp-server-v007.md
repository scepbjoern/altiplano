...
- [ ] OAuth 2.1-Authentifizierung: Vollständiger Authorization Code Flow mit PKCE via FastMCP `OAuthProvider` mit Custom SQLite-Token-Store. Ermöglicht Anbindung aller grossen Web-LLM-Clients (ChatGPT Web, Claude Web, Gemini u. a.).
...
- [ ] Externer Identity Provider (Auth0, Casdoor, Google, GitHub) für OAuth 2.1 – der selbstverwaltete SQLite-Token-Store ist für Single-User-Betrieb ausreichend.
...
| US-9 | Als Nutzer möchte ich den MCP-Server über ChatGPT Web, Claude Web oder Gemini nutzen, indem ich mich einmalig via OAuth 2.1-Login autorisiere. | Medium | Web-LLM-Client kann sich via OAuth 2.1 Authorization Code + PKCE am MCP-Server anmelden und anschliessend Tools aufrufen. |
...
| OAuth 2.1 Authentifizierung | Vollständiger Authorization Code Flow + PKCE via FastMCP `OAuthProvider`. Custom SQLite-Token-Store für Clients, Authorization Codes, Access Tokens und Refresh Tokens. Exponiert `/.well-known/oauth-protected-resource` und `/.well-known/oauth-authorization-server`. | Medium | Must | KI-Client (Web) | Ermöglicht Anbindung aller Web-LLM-Clients (ChatGPT Web, Claude Web, Gemini u. a.). FastMCP `OAuthProvider` ist ein Framework-Hook: Storage, User Management und Token-Logik werden selbst implementiert. |
...
| OAuth Client | Registrierter MCP-Client (z. B. ChatGPT, Claude Web) | `client_id`, `client_secret`, `redirect_uris`, `scopes` | Wird im SQLite-Token-Store verwaltet. | Medium |
| OAuth Token | Access- und Refresh-Token für autorisierte Clients | `token`, `client_id`, `scopes`, `expires_at`, `revoked` | Verknüpft mit OAuth Client. | Medium |
...
| MCP Client (remote) | eingehend | MCP Protocol / HTTP SSE | Anbindung von Web-LLM-Clients | - | OAuth 2.1 Authorization Code + PKCE; Redirect URIs je Client (z. B. ChatGPT Web: `https://chatgpt.com/aip/mcp/oauth/callback`; Claude Web: gemäss Anthropic-Doku) |
| OAuth Discovery Endpunkte | eingehend | HTTP GET | Automatische Client-Registrierung (DCR) und Auth-Server-Erkennung | - | `/.well-known/oauth-protected-resource`, `/.well-known/oauth-authorization-server` |
...
|---|---|---|
| MCP Server (FastMCP) | genutzt | Haupt-Framework. `OAuthProvider` für Medium-Version vorgesehen. |
| HTTP-Client (httpx) | genutzt | Kommunikation mit Vikunja-API. |
| pytest | genutzt | Für Unit- und Integrationstests. |
| SQLite | geplant (Medium) | Token-Store für OAuth 2.1 (Authorization Codes, Access Tokens, Refresh Tokens, Clients). |
...
- **OAuth 2.1 Remote-Authentifizierung (eingehend):**
  - Web-LLM-Clients (ChatGPT Web, Claude Web, Gemini u. a.) authentifizieren sich via OAuth 2.1 Authorization Code Flow + PKCE am MCP-Server.
  - Implementiert via FastMCP `OAuthProvider` (abstrakter Framework-Hook): Der MCP-Server wird selbst Authorization Server und Resource Server.
  - **Custom SQLite-Token-Store:** Der Entwickler implementiert selbst alle sicherheitsrelevanten Methoden: `get_client`, `register_client`, `authorize`, `load_authorization_code`, `exchange_authorization_code`, `load_access_token`, `verify_token`, `load_refresh_token`, `exchange_refresh_token`, `revoke_token`.
  - **Sicherheitspflichten bei der Implementierung:** PKCE-Validierung, strikte Redirect-URI-Validierung, Token-Hashing (kein Klartext-Token-Storage), Ablaufzeiten (Access Token kurz, Refresh Token länger), Refresh-Token-Rotation, Scope-Prüfung bei jedem Request.
  - Der Cloudflare Tunnel bleibt als Transport-Layer erhalten (HTTPS), ersetzt aber nicht die Authentifizierung.
...
| Web-Client OAuth Login | User fügt MCP-Server-URL in ChatGPT Web / Claude Web / Gemini hinzu. Client leitet automatisch zum OAuth-Login weiter. Nach Autorisierung sind MCP-Tools im Web-Client nutzbar. | US-9 | Medium | OAuth-Flow läuft vollständig durch; Web-LLM-Client kann MCP-Tools aufrufen. |
...
| Risiko | FastMCP `OAuthProvider` ist ein Framework-Hook, kein fertiger Auth-Server. Alle sicherheitsrelevanten Aspekte (Token-Hashing, PKCE-Validierung, Redirect-URI-Prüfung, Token-Ablaufzeiten, Refresh-Token-Rotation, Scope-Prüfung, Revocation) müssen selbst korrekt implementiert werden. Fehler hier sind sicherheitskritisch. | Hoch | Implementierung sorgfältig planen; FastMCP-Dokumentation und MCP-Auth-Spezifikation als verbindliche Referenz nutzen; Security-Review vor Produktivbetrieb einplanen. |
| Risiko | Redirect URIs der Web-LLM-Clients (ChatGPT Web, Claude Web, Gemini) können sich ändern oder unterscheiden sich je Anbieter. Falsche Redirect URIs blockieren den OAuth-Flow komplett. | Hoch | Redirect URIs vor der Implementierung je Client recherchieren und dokumentieren. Im Token-Store müssen diese als konfigurierbare Whitelist hinterlegt werden. |
...
| Offene Frage | Redirect URIs der verschiedenen Web-LLM-Clients (ChatGPT Web, Claude Web, Gemini) müssen vor dem Feature-Plan recherchiert und dokumentiert werden. | Hoch | Im Rahmen des `/plan-feature oauth-authentication`-Workflows klären. |
...
| OAuth 2.1 Authentifizierung | Authorization Code Flow + PKCE via FastMCP `OAuthProvider` mit Custom SQLite-Token-Store. Discovery-Endpunkte, Redirect-URI-Whitelist je Web-LLM-Client (ChatGPT Web, Claude Web, Gemini u. a.). | Medium | Remote HTTP MCP deployed. | 7b |
...
- **FastMCP OAuthProvider Referenz:** FastMCP `OAuthProvider` ist eine abstrakte Klasse. Storage, User Management und Token-Logik werden vollständig an die eigene Implementierung delegiert. Kein fertiger lokaler Auth-Server; `StaticTokenVerifier` (einfacher Bearer-Token) ist ausdrücklich nur für Entwicklung/Testing gedacht und **nicht** für Web-LLM-Clients geeignet, da diese den vollständigen OAuth-Discovery-Flow benötigen.
