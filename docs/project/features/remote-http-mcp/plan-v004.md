...
**Confidence Score:** 9/10 (Review R01 integriert, alle Architekturfragen und Deploy-Grenzfälle wie Host-Binding und DNS-Rebinding sind adressiert. Abhängigkeit zu OAuth-Feature dokumentiert.)
...
| Abhängigkeiten | mcp (bringt uvicorn & starlette bereits mit); OAuth 2.1 Feature (später, siehe "Feature-Abhängigkeiten") |
...
| v004 | 2026-06-25 | Abhängigkeits-Dokumentation | Abhängigkeit zum zukünftigen OAuth 2.1 Feature dokumentiert. Hinweis auf später erforderliche Integration und Nacharbeiten hinzugefügt. Feature-Status korrigiert: `done` statt `planned`. |
...
- OAuth 2.1 Integration (siehe "Feature-Abhängigkeiten").
...
- **Zukünftig (mit OAuth):** Authentifizierung über OAuth 2.1 statt statischer Reverse-Proxy-Header. Wird in separatem oauth-authentication Feature umgesetzt.

## Feature-Abhängigkeiten

### Abhängigkeit zum OAuth 2.1 Feature

Das neue Feature "OAuth 2.1 Authentifizierung" (`docs/project/features/oauth-authentication/plan-v001.md`) wird später eine **Integration mit dem Remote-HTTP-Transport** erfordern:

- **Schnittstelle:** Die `main()`-Funktion in `server.py` wird bei OAuth-Implementierung um OAuth-Provider-Logik erweitert (nur für `sse` / `streamable-http`, nicht für `stdio`).
- **Betroffene Dateien:** `src/altiplano/server.py` (Task 4 im OAuth-Plan referenziert direkt die `main()`-Weiche dieses Features).
- **Keine Code-Änderung jetzt:** Der aktuelle Remote-HTTP-MCP-Plan bleibt unverändert. Die OAuth-Integration erfolgt als Teil des OAuth-Feature-Plans.
- **Nacharbeit erforderlich:** Nach Abschluss des OAuth-Features muss der Remote-HTTP-MCP-Plan überarbeitet werden, um die Integration zu dokumentieren. Ggf. ist auch eine Revision dieses Plans erforderlich (neue v-Version).
...
### Bestehende Dateien
- `src/altiplano/server.py` - UPDATE: `main()` Funktion umbauen (Transport-Weiche). **Zukünftig (mit OAuth):** Wird um OAuth-Provider-Logik erweitert.
...
**GOTCHA:** FastMCP >= 1.27 hat einen DNS-Rebinding-Schutz. Wenn `mcp.run()` keine externen Hosts zulässt, muss das hier evtl. konfiguriert werden (z.B. über `mcp.settings` oder beim `FastMCP` Konstruktoraufruf). **Zukünftig:** OAuth-Provider wird hier eingefügt.  
**ACCEPTANCE CRITERIA:**
...
**IMPORTS:** n/a.  
**GOTCHA:** DNS-Rebinding über Proxy. Ggf. in `.env.example` Cloudflare Header dokumentieren. **Zukünftig:** OAuth-Variablen (z.B. `OAUTH_ADMIN_PASSWORD`) müssen hinzugefügt werden.  
**ACCEPTANCE CRITERIA:**
...
- [x] `server.py` ist für `sse` und `streamable-http` vorbereitet und getestet.
- [x] Deployment erfolgt sauber über `deploy/docker-compose.yml`.
- [x] Container bindet korrekt auf `0.0.0.0` und läuft als Non-Root-User.
- [x] Alle Unit-Tests bestehen.
- [x] DNS-Rebinding-Schutz bei Reverse-Proxy Einsatz ist evaluiert/behoben.

## Completion Checklist

- [x] Alle Tasks sind umgesetzt und validiert.
- [x] Plan-/PRD-Abweichungen sind dokumentiert und genehmigt.
- [x] Feature ist bereit für `/document` und `/commit`.
- [ ] **Nach Abschluss des OAuth 2.1 Features:** Remote HTTP MCP Plan überarbeiten, um OAuth-Integration zu dokumentieren (neue Plan-Version erforderlich).
...
- **Zukünftig:** Nach OAuth-Implementierung auch die OAuth-Login-Flow-Dokumentation hinzufügen.

## Offene Fragen

- **OAuth-Integration:** Nach Abschluss des OAuth 2.1 Features wird dieser Plan überarbeitet werden, um die Integration mit der `main()`-Funktion zu dokumentieren. Die `server.py` wird dann OAuth-Provider-Logik für Remote-Transport enthalten (siehe OAuth-Plan Task 4). Eine neue Plan-Version (v005?) wird wahrscheinlich erforderlich.
