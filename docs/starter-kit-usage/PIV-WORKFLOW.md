...
Wenn mehrere Personen im selben Repository an einem gemeinsamen IT-System arbeiten, gilt weiterhin dieser PIV-Ablauf. Beachtet zusätzlich die Koordinationsregeln in [`COLLABORATION.md`](COLLABORATION.md), insbesondere zu gemeinsamem PRD, Feature-Aufteilung, `TASKS.md`, Branches und Code-Struktur.
...
| `adapt-to-project` | Einmalig nach PRD-Review, Review-Integration und fachlicher PRD-Bestätigung, vor dem ersten `plan-feature`: bereinigt Demo-Code auf Basis des PRDs und validiert den Code mit Tests | `/adapt-to-project docs/project/prds/antragssystem-v002.md` |
...
Die Skills werden automatisch durch das IDE-Plugin / den Agenten aus dem Ordner `.agents/skills/` geladen, sobald VS Code geöffnet wird. Es ist kein manueller Setup-Schritt mehr nötig. Starte einfach direkt mit `/prime` im Chat.
...
Im CAS-Kontext ist das Starter-Kit-Projekt Brownfield: Der technische Stack ist bereits durch das Starter Kit und die Projektregeln vorgegeben. Das PRD soll deshalb keine neuen Stack-Entscheide erfinden, sondern auf vorhandene Vorgaben wie `AGENTS.md`, `KILO_INSTRUCTIONS.md`, `pyproject.toml`, `src/altiplano/server.py` und bestehende Starter-Kit-Konventionen referenzieren. Wenn euer konkretes System bestimmte Bausteine des MCP-Servers nicht benötigt, ist das im Prototyp akzeptabel. Nicht benötigte Tools können gelöscht oder ignoriert werden.
...
Der Agent liest Projektregeln, `AGENTS.md`, `TASKS.md`, `pyproject.toml` und wichtige Dateien. Danach bekommst du eine kurze Übersicht über den aktuellen Stand.
...
Der Skill liest den Abschnitt "Starter Kit Nutzung" aus dem bestätigten PRD, schlägt vor, welche ungenutzten MCP-Tools aus `server.py` entfernt werden können, und führt die Bereinigung nach deiner Bestätigung durch. Am Ende validiert er den Code automatisch durch Tests.

Nach der Bereinigung: Führe `uv run pytest` aus und prüfe, ob die Tests noch erfolgreich durchlaufen.
...
Für das Laufzeitverhalten des MCP-Servers startest du zusätzlich den Server lokal (Anleitung siehe `MCP_LOCAL_SETUP.md`) und testest ihn beispielsweise im Claude Desktop, mit dem MCP Inspector oder einem Testskript. Genaueres hierzu findest du in `TESTING.md` und `MCP_LOCAL_SETUP.md`.
